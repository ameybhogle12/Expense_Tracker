...
*   **Itemized Bill Splitter (Quick Mode & Fun Mode):** Split complex dining/group bills item-by-item (e.g. pastries, drinks, desserts) with custom per-item member assignments, optional proportional tax & tip calculation, live per-person breakdown, and an interactive Fun Mode teaser tab featuring the `Wait_Guys.png` image with synced GTA crying audio (`gta-san-andreas-male-panic-crying-scream.mp3`).
...
### 15. Reminder/Insight Notifications Never Fire While the App is Closed
*   **Problem:** Daily spend reminders, spending insights and upcoming subscription/EMI reminders only arrived while the app was open. With the app closed or swiped away, nothing was delivered.
*   **Root Cause:** Every one of these notifications was posted with `FlutterLocalNotificationsPlugin.show()`, which fires *immediately and in-process*. The decision to fire lived in `LogProcessor.processAll()`, reachable from only two places, both of which require a live Dart isolate: (1) the `Timer.periodic(1 minute)` in `ExpenseProvider` — this in-app poll was the only thing actually delivering notifications, hence "works when open"; (2) the 15-minute Workmanager task. Workmanager is *deferrable* — Doze and App Standby batch it, and most OEM skins (Xiaomi, Realme, Oppo, Vivo, Samsung) force-stop the process on swipe-away, which cancels its pending jobs until the next manual launch. So with the app closed there was nothing registered with the OS that could wake the device and post anything. Compounding this, the app could not have used scheduled notifications even if asked: since v16 the plugin declares only `POST_NOTIFICATIONS`/`VIBRATE` in its own manifest, and the app declared neither `ScheduledNotificationReceiver` (alarm fires, nothing posts the notification) nor `ScheduledNotificationBootReceiver` (all pending alarms lost on reboot/update).
*   **Solution:** Moved delivery to the OS AlarmManager, which does not care whether the app process exists. Declared both plugin receivers in `AndroidManifest.xml`. Added `zonedSchedule`-based scheduling to `NotificationService` and a new `NotificationScheduler` that arms a rolling 7-day daily nudge, the next spending insight, and per-subscription/per-EMI due reminders under deterministic IDs, so re-arming overwrites instead of accumulating. `rearmAll()` runs on launch, on resume, on `AppLifecycleState.paused` (the last chance before the process may be killed) and from the Workmanager task. Workmanager is retained for the data work it is actually suited to — auto-logging subscriptions and EMIs — but is no longer the delivery path for notifications.
*   **Notes:** Uses `AndroidScheduleMode.inexactAllowWhileIdle`, which still punches through Doze but needs no exact-alarm permission — `USE_EXACT_ALARM` is audited by Play and intended for alarm-clock/calendar apps. Alarms carry fixed text and cannot run Dart at fire time, so the insight body is composed when arming (refreshed on every pause/resume/background run). Alarms are pinned to `tz.UTC` because they are scheduled as absolute instants, which avoids bundling the 445 KB timezone database; `DateTimeComponents` repeats are deliberately unused since they re-derive wall-clock time in the supplied zone and would drift by the UTC offset. The existing `-keep class com.dexterous.**` ProGuard rule is load-bearing here — the plugin Gson-serializes `NotificationDetails` into SharedPreferences to reschedule after reboot.
